...
const themesDirs = fs
  ...
  .map((dir) => dir.name);
for (const themesDir of themesDirs) {
  const keycloakTemplates = [];
  const pagesDirs = fs
    .readdirSync(path.join(paths.keycloakThemesPath, themesDir))
    .filter((dir) => dir.endsWith("-pages"));
  for (const pagesDir of pagesDirs) {
    const templatesDirs = fs
      .readdirSync(path.join(paths.keycloakThemesPath, themesDir, pagesDir))
      .filter((dir) => dir.endsWith("-template"));

    for (const templatesDir of templatesDirs) {
      const ftlTemplates = fs
        .readdirSync(
          path.join(paths.keycloakThemesPath, themesDir, pagesDir, templatesDir)
        )
