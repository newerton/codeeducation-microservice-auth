...
module.controller(
  ...
  (Dialog, $scope, Current, Notifications, $location) => {
    $scope.removeClient = () => {
      Dialog.confirmDelete($scope.client.clientId, 'client', () => {
        $scope.client.$remove(
          {
...
module.controller(
  ...
  (
    ...
    $scope.searchQuery = () => {
      ...
      $scope.roles = ClientRoleList.query($scope.query, () => {
  ...
  (
    ...
    var updateCurrentPartial = (val) => {
      ...
      switch (val) {
        ...
        case 'client-x509':
          ...
          break;
        default:
          $scope.currentAuthenticatorConfigProperties =
            clientConfigProperties[val];
          $scope.clientAuthenticatorConfigPartial =
    ...
    $scope.$watch(
      ...
      () => {
        if (
          !angular.equals(
            $scope.client.clientAuthenticatorType,
            client.clientAuthenticatorType,
          )
        ) {
          Client.update(
    ...
    $scope.regenerateRegistrationAccessToken = () => {
      var _secret = ClientRegistrationAccessToken.update(
        { realm: $scope.realm.realm, client: $scope.client.id },
        (data) => {
          Notifications.success(
            'The registration access token has been updated.',
          );
          $scope.client['registrationAccessToken'] =
            data.registrationAccessToken;
        },
        () => {
...
module.controller(
  'ClientSecretCtrl',
  ($scope, $location, Client, ClientSecret, Notifications) => {
    var secret = ClientSecret.get(
      { realm: $scope.realm.realm, client: $scope.client.id },
      () => {
    ...
    $scope.changePassword = () => {
      var secret = ClientSecret.update(
        { realm: $scope.realm.realm, client: $scope.client.id },
        () => {
    ...
    $scope.tokenEndpointAuthSigningAlg =
      $scope.client.attributes['token.endpoint.auth.signing.alg'];
    ...
    $scope.cancel = () => {
      $location.url(
        `/realms/${$scope.realm.realm}/clients/${$scope.client.id}/credentials`,
      );
    };
  },
);
...
module.controller(
  'ClientX509Ctrl',
  ($scope, $location, Client, Notifications) => {
    ...
    $scope.save = () => {
      if (!$scope.client.attributes['x509.subjectdn']) {
        ...
        Client.update(
          ...
          () => {
            Notifications.error(
              'The SubjectDN was not changed due to a problem.',
            );
    ...
    $scope.reset = () => {
      $scope.client.attributes['x509.subjectdn'] =
        $scope.clientCopy.attributes['x509.subjectdn'];
      $location.url(
        `/realms/${$scope.realm.realm}/clients/${$scope.client.id}/credentials`,
      );
    };
...
module.controller(
  'ClientSignedJWTCtrl',
  ($scope, $location, Client, ClientCertificate, Notifications, $route) => {
    var signingKeyInfo = ClientCertificate.get(
      {
        realm: $scope.realm.realm,
        client: $scope.client.id,
    ...
    $scope.importCertificate = () => {
      $location.url(
        `/realms/${$scope.realm.realm}/clients/${$scope.client.id}/credentials/client-jwt/Signing/import/jwt.credential`,
      );
    };

    $scope.generateSigningKey = () => {
      $location.url(
        `/realms/${$scope.realm.realm}/clients/${$scope.client.id}/credentials/client-jwt/Signing/export/jwt.credential`,
      );
    };
...
module.controller(
  'ClientGenericCredentialsCtrl',
  ($scope, _$location, Client, Notifications) => {
...
module.controller(
  ...
  (
    ...
    if ($scope.client.identityProviders) {
      _length = $scope.client.identityProviders.length;
      ...
      for (i = 0; i < $scope.client.identityProviders.length; i++) {
        ...
        if (clientProvider.retrieveToken) {
          clientProvider.retrieveToken =
            clientProvider.retrieveToken.toString();
        }
      }
    } else {
      ...
      for (i = 0; i < $scope.client.identityProviders.length; i++) {
        ...
        if (clientProvider) {
          if (clientProvider.id === identityProvider.id) {
            ...
            $scope.identityProviders[i].retrieveToken =
              clientProvider.retrieveToken;
            break;
          }
...
module.controller(
  ...
  (
    ...
    client,
    ClientCertificate,
    ClientCertificateGenerate,
    _ClientCertificateDownload,
    Notifications,
  ) => {
    $scope.realm = realm;
    $scope.client = client;
    ...
    var signingKeyInfo = ClientCertificate.get(
      { realm: realm.realm, client: client.id, attribute: 'saml.signing' },
      () => {
        $scope.signingKeyInfo = signingKeyInfo;
      },
    );
    ...
    $scope.generateSigningKey = () => {
      var keyInfo = ClientCertificateGenerate.generate(
        { realm: realm.realm, client: client.id, attribute: 'saml.signing' },
        () => {
    ...
    $scope.importSigningKey = () => {
      $location.url(
        `/realms/${realm.realm}/clients/${client.id}/saml/Signing/import/saml.signing`,
      );
    };

    $scope.exportSigningKey = () => {
      $location.url(
        `/realms/${realm.realm}/clients/${client.id}/saml/Signing/export/saml.signing`,
      );
    };

    var encryptionKeyInfo = ClientCertificate.get(
      { realm: realm.realm, client: client.id, attribute: 'saml.encryption' },
      () => {
        $scope.encryptionKeyInfo = encryptionKeyInfo;
      },
    );

    $scope.generateEncryptionKey = () => {
      var keyInfo = ClientCertificateGenerate.generate(
        { realm: realm.realm, client: client.id, attribute: 'saml.encryption' },
        () => {
    ...
    $scope.importEncryptionKey = () => {
      $location.url(
        `/realms/${realm.realm}/clients/${client.id}/saml/Encryption/import/saml.encryption`,
      );
    };

    $scope.exportEncryptionKey = () => {
      $location.url(
        `/realms/${realm.realm}/clients/${client.id}/saml/Encryption/export/saml.encryption`,
      );
    };
...
module.controller(
  ...
  (
    ...
    if (callingContext === 'saml') {
      var _uploadUrl = `${authUrl}/admin/realms/${realm.realm}/clients/${client.id}/certificates/${attribute}/upload`;
      var _redirectLocation = `/realms/${realm.realm}/clients/${client.id}/saml/keys`;
    ...
    $scope.keyFormats = ['JKS', 'PKCS12', 'Certificate PEM'];
    ...
    $scope.hideKeystoreSettings = () =>
      $scope.uploadKeyFormat === 'Certificate PEM' ||
      $scope.uploadKeyFormat === 'Public Key PEM' ||
      $scope.uploadKeyFormat === 'JSON Web Key Set';
      ...
      for (var i = 0; i < $scope.files.length; i++) {
        ...
        $scope.upload = $upload
          .upload({
            ...
            data: {
              keystoreFormat: $scope.uploadKeyFormat,
...
module.controller(
  ...
  (
    ...
    if (callingContext === 'saml') {
      var _downloadUrl = `${authUrl}/admin/realms/${realm.realm}/clients/${client.id}/certificates/${attribute}/download`;
      var _realmCertificate = true;
    } else if (callingContext === 'jwt-credentials') {
    ...
    $scope.keyFormats = ['JKS', 'PKCS12'];

    var keyInfo = ClientCertificate.get(
      { realm: realm.realm, client: client.id, attribute: attribute },
      () => {
    ...
    $scope.download = () => {
      $http({
        ...
        .then((response) => {
          ...
          if (callingContext === 'jwt-credentials') {
            $location.url(
              `/realms/${realm.realm}/clients/${client.id}/credentials`,
            );
            Notifications.success(
...
module.controller(
  'ClientSessionsCtrl',
  ($scope, realm, sessionCount, client, ClientUserSessions) => {
    $scope.realm = realm;
    ...
    $scope.toDate = (val) => new Date(val);

    $scope.loadUsers = () => {
      ClientUserSessions.query($scope.query, (updated) => {
...
module.controller(
  'ClientOfflineSessionsCtrl',
  ($scope, realm, offlineSessionCount, client, ClientOfflineSessions) => {
    $scope.realm = realm;
    ...
    $scope.loadUsers = () => {
      ClientOfflineSessions.query($scope.query, (updated) => {
        $scope.sessions = updated;
      });
    };
  },
      ...
      if ($scope.create) {
        ClientRole.save(
          ...
          (_data, _headers) => {
            ...
            ClientRole.get(
              { realm: realm.realm, client: client.id, role: role.name },
              (role) => {
                var id = role.id;
                $location.url(
                  `/realms/${realm.realm}/clients/${client.id}/roles/${id}`,
                );
...
module.controller(
  'ClientRoleMembersCtrl',
  (
    $scope,
    realm,
    client,
    role,
    ClientRoleMembership,
    _Dialog,
    _Notifications,
    _$location,
  ) => {
    ...
    $scope.searchQuery = () => {
      ...
      $scope.users = ClientRoleMembership.query($scope.query, () => {
...
module.controller(
  'ClientImportCtrl',
  ($scope, $location, $upload, realm, _serverInfo, Notifications) => {
    $scope.realm = realm;
    ...
    $scope.uploadFile = () => {
      ...
      for (var i = 0; i < $scope.files.length; i++) {
        ...
        $scope.upload = $upload
          ...
          .success((_data, _status, _headers) => {
...
module.controller(
  ...
  (
    ...
    $scope.searchQuery = () => {
      ...
      $scope.clients = Client.query($scope.query, () => {
        $scope.searchLoaded = true;
        $scope.lastSearch = $scope.query.search;
    ...
    $scope.removeClient = (client) => {
      Dialog.confirmDelete(client.clientId, 'client', () => {
        Client.remove(
          {
            realm: realm.realm,
            client: client.id,
    ...
    $scope.exportClient = (client) => {
      ...
      saveAs(
        new Blob([angular.toJson(clientCopy, 4)], { type: 'application/json' }),
...
module.controller(
  ...
  (
    ...
    $scope.changeFormat = () => {
      var url = ClientInstallation.url({
        realm: $routeParams.realm,
        client: $routeParams.client,
        provider: $scope.configFormat.id,
      });
    ...
    $scope.download = () => {
      saveAs(
        new Blob([$scope.installation], {
          type: $scope.configFormat.mediaType,
        }),
        $scope.configFormat.filename,
      );
    };
  },
);
...
module.controller(
  ...
  (
    ...
    $scope.accessTypes = ['confidential', 'public', 'bearer-only'];
    ...
    $scope.nameIdFormats = ['username', 'email', 'transient', 'persistent'];
    ...
    $scope.changePkceCodeChallengeMethodOptions = ['S256', 'plain', ''];
    ...
    $scope.accessTokenLifespan = TimeUnit2.asUnit(
      client.attributes['access.token.lifespan'],
    );
    $scope.samlAssertionLifespan = TimeUnit2.asUnit(
      client.attributes['saml.assertion.lifespan'],
    );
    $scope.clientSessionIdleTimeout = TimeUnit2.asUnit(
      client.attributes['client.session.idle.timeout'],
    );
    $scope.clientSessionMaxLifespan = TimeUnit2.asUnit(
      client.attributes['client.session.max.lifespan'],
    );
    $scope.clientOfflineSessionIdleTimeout = TimeUnit2.asUnit(
      client.attributes['client.offline.session.idle.timeout'],
    );
    $scope.clientOfflineSessionMaxLifespan = TimeUnit2.asUnit(
      client.attributes['client.offline.session.max.lifespan'],
    );
    $scope.oauth2DeviceCodeLifespan = TimeUnit2.asUnit(
      client.attributes['oauth2.device.code.lifespan'],
    );
    $scope.oauth2DevicePollingInterval = parseInt(
      client.attributes['oauth2.device.polling.interval'],
      10,
    );
    ...
    if (client.origin) {
      if ($scope.access.viewRealm) {
        Components.get(
          { realm: realm.realm, componentId: client.origin },
          (link) => {
        ...
        // KEYCLOAK-4328
        ClientStorageOperations.simpleName.get(
          { realm: realm.realm, componentId: client.origin },
          (link) => {
            $scope.originName = link.name;
    ...
    function updateProperties() {
      ...
      if ($scope.client.protocol) {
        $scope.protocol =
          $scope.protocols[$scope.protocols.indexOf($scope.client.protocol)];
      } else {
      ...
      if ($scope.client.attributes['saml.signature.algorithm'] === 'RSA_SHA1') {
        ...
        $scope.client.attributes['saml.signature.algorithm'] === 'RSA_SHA256'
      ) {
        $scope.signatureAlgorithm = $scope.signatureAlgorithms[1];
      } else if (
        $scope.client.attributes['saml.signature.algorithm'] ===
        'RSA_SHA256_MGF1'
      ) {
        $scope.signatureAlgorithm = $scope.signatureAlgorithms[2];
      } else if (
        $scope.client.attributes['saml.signature.algorithm'] === 'RSA_SHA512'
      ) {
        ...
        $scope.client.attributes['saml.signature.algorithm'] === 'DSA_SHA1'
      ) {
      ...
      if ($scope.client.attributes['saml_name_id_format'] === 'username') {
        ...
        $scope.client.attributes['saml_name_id_format'] === 'transient'
      ) {
        $scope.nameIdFormat = $scope.nameIdFormats[2];
      } else if (
        $scope.client.attributes['saml_name_id_format'] === 'persistent'
      ) {
      ...
      if ($scope.client.attributes['saml.server.signature.keyinfo.ext']) {
        if (
          $scope.client.attributes['saml.server.signature.keyinfo.ext'] ===
          'true'
        ) {
      ...
      $scope.accessTokenSignedResponseAlg =
        $scope.client.attributes['access.token.signed.response.alg'];
      $scope.idTokenSignedResponseAlg =
        $scope.client.attributes['id.token.signed.response.alg'];
      $scope.idTokenEncryptedResponseAlg =
        $scope.client.attributes['id.token.encrypted.response.alg'];
      $scope.idTokenEncryptedResponseEnc =
        $scope.client.attributes['id.token.encrypted.response.enc'];

      var attrVal1 =
        $scope.client.attributes['user.info.response.signature.alg'];
      $scope.userInfoSignedResponseAlg =
        attrVal1 == null ? 'unsigned' : attrVal1;
      ...
      $scope.requestObjectRequired =
        attrVal3 == null ? 'not required' : attrVal3;

      var attrVal4 = $scope.client.attributes['pkce.code.challenge.method'];
      $scope.pkceCodeChallengeMethod = attrVal4 == null ? 'none' : attrVal4;

      if (
        $scope.client.attributes['exclude.session.state.from.auth.response']
      ) {
        if (
          $scope.client.attributes[
      ...
      if (
        $scope.client.attributes['oauth2.device.authorization.grant.enabled']
      ) {
        if (
          $scope.client.attributes[
      ...
      // https://tools.ietf.org/html/draft-ietf-oauth-mtls-08#section-3
      if (
        $scope.client.attributes['tls.client.certificate.bound.access.tokens']
      ) {
        if (
          $scope.client.attributes[
      ...
      var useRefreshToken =
        $scope.client.attributes['client_credentials.use_refresh_token'];
      ...
      if ($scope.client.attributes['backchannel.logout.session.required']) {
        if (
          $scope.client.attributes['backchannel.logout.session.required'] ===
          'true'
        ) {
      ...
      if (
        $scope.client.attributes['backchannel.logout.revoke.offline.tokens']
      ) {
        if (
          $scope.client.attributes[
      ...
      if (
        $scope.client.attributes['request.uris'] &&
        $scope.client.attributes['request.uris'].length > 0
      ) {
        $scope.client.requestUris =
          $scope.client.attributes['request.uris'].split('##');
      } else {
        $scope.client.requestUris = [];
      }
    }
    ...
    $scope.samlIdpInitiatedUrl = (ssoName) =>
      encodeURI(
        `${$location.absUrl().replace(/\/admin.*/, '/realms/') + realm.realm}/protocol/saml/clients/`,
      ) + encodeURIComponent(ssoName);
    ...
    $scope.changeAlgorithm = () => {
      $scope.clientEdit.attributes['saml.signature.algorithm'] =
        $scope.signatureAlgorithm;
    };

    $scope.changeNameIdFormat = () => {
      $scope.clientEdit.attributes['saml_name_id_format'] = $scope.nameIdFormat;
    };

    $scope.changeSamlSigKeyNameTranformer = () => {
      $scope.clientEdit.attributes[
        'saml.server.signature.keyinfo.xmlSigKeyInfoKeyNameTransformer'
      ] = $scope.samlXmlKeyNameTranformer;
    };

    $scope.changeAccessTokenSignedResponseAlg = () => {
      $scope.clientEdit.attributes['access.token.signed.response.alg'] =
        $scope.accessTokenSignedResponseAlg;
    };

    $scope.changeIdTokenSignedResponseAlg = () => {
      $scope.clientEdit.attributes['id.token.signed.response.alg'] =
        $scope.idTokenSignedResponseAlg;
    };

    $scope.changeIdTokenEncryptedResponseAlg = () => {
      $scope.clientEdit.attributes['id.token.encrypted.response.alg'] =
        $scope.idTokenEncryptedResponseAlg;
    };

    $scope.changeIdTokenEncryptedResponseEnc = () => {
      $scope.clientEdit.attributes['id.token.encrypted.response.enc'] =
        $scope.idTokenEncryptedResponseEnc;
    };

    $scope.changeUserInfoSignedResponseAlg = () => {
      if ($scope.userInfoSignedResponseAlg === 'unsigned') {
        $scope.clientEdit.attributes['user.info.response.signature.alg'] = null;
      } else {
        $scope.clientEdit.attributes['user.info.response.signature.alg'] =
          $scope.userInfoSignedResponseAlg;
      }
    };

    $scope.changeRequestObjectSignatureAlg = () => {
      if ($scope.requestObjectSignatureAlg === 'any') {
        $scope.clientEdit.attributes['request.object.signature.alg'] = null;
      } else {
        $scope.clientEdit.attributes['request.object.signature.alg'] =
          $scope.requestObjectSignatureAlg;
      }
    };

    $scope.changeRequestObjectRequired = () => {
      if ($scope.requestObjectRequired === 'not required') {
        $scope.clientEdit.attributes['request.object.required'] = null;
      } else {
        $scope.clientEdit.attributes['request.object.required'] =
          $scope.requestObjectRequired;
      }
    };

    $scope.changePkceCodeChallengeMethod = () => {
      $scope.clientEdit.attributes['pkce.code.challenge.method'] =
        $scope.pkceCodeChallengeMethod;
    };
    ...
    $scope.updateTimeouts = () => {
      if ($scope.accessTokenLifespan.time) {
        if ($scope.accessTokenLifespan.time === -1) {
          ...
          $scope.clientEdit.attributes['access.token.lifespan'] =
            $scope.accessTokenLifespan.toSeconds();
        }
      } else {
    ...
    $scope.updateAssertionLifespan = () => {
      if ($scope.samlAssertionLifespan.time) {
        $scope.clientEdit.attributes['saml.assertion.lifespan'] =
          $scope.samlAssertionLifespan.toSeconds();
      } else {
        $scope.clientEdit.attributes['saml.assertion.lifespan'] = null;
      }
    };

    $scope.updateClientSessionIdleTimeout = () => {
      if ($scope.clientSessionIdleTimeout.time) {
        $scope.clientEdit.attributes['client.session.idle.timeout'] =
          $scope.clientSessionIdleTimeout.toSeconds();
      } else {
        $scope.clientEdit.attributes['client.session.idle.timeout'] = null;
      }
    };

    $scope.updateClientSessionMaxLifespan = () => {
      if ($scope.clientSessionMaxLifespan.time) {
        $scope.clientEdit.attributes['client.session.max.lifespan'] =
          $scope.clientSessionMaxLifespan.toSeconds();
      } else {
        $scope.clientEdit.attributes['client.session.max.lifespan'] = null;
      }
    };

    $scope.updateClientOfflineSessionIdleTimeout = () => {
      if ($scope.clientOfflineSessionIdleTimeout.time) {
        $scope.clientEdit.attributes['client.offline.session.idle.timeout'] =
          $scope.clientOfflineSessionIdleTimeout.toSeconds();
      } else {
        $scope.clientEdit.attributes['client.offline.session.idle.timeout'] =
          null;
      }
    ...
    $scope.updateClientOfflineSessionMaxLifespan = () => {
      if ($scope.clientOfflineSessionMaxLifespan.time) {
        $scope.clientEdit.attributes['client.offline.session.max.lifespan'] =
          $scope.clientOfflineSessionMaxLifespan.toSeconds();
      } else {
        $scope.clientEdit.attributes['client.offline.session.max.lifespan'] =
          null;
      }
    ...
    $scope.updateOauth2DeviceCodeLifespan = () => {
      if ($scope.oauth2DeviceCodeLifespan.time) {
        $scope.clientEdit.attributes['oauth2.device.code.lifespan'] =
          $scope.oauth2DeviceCodeLifespan.toSeconds();
      } else {
        $scope.clientEdit.attributes['oauth2.device.code.lifespan'] = null;
      }
    };

    $scope.updateOauth2DevicePollingInterval = () => {
      if ($scope.oauth2DevicePollingInterval) {
        $scope.clientEdit.attributes['oauth2.device.polling.interval'] =
          $scope.oauth2DevicePollingInterval;
      } else {
    ...
    function configureAuthorizationServices() {
      ...
      if (
        $scope.client.authorizationServicesEnabled &&
        !$scope.clientEdit.authorizationServicesEnabled
      ) {
        Dialog.confirm(
    ...
    $scope.save = () => {
      ...
      if (
        $scope.clientEdit.requestUris &&
        $scope.clientEdit.requestUris.length > 0
      ) {
        $scope.clientEdit.attributes['request.uris'] =
          $scope.clientEdit.requestUris.join('##');
      } else {
      ...
      if ($scope.samlServerSignatureEnableKeyInfoExtension === true) {
        $scope.clientEdit.attributes['saml.server.signature.keyinfo.ext'] =
          'true';
      } else {
        $scope.clientEdit.attributes['saml.server.signature.keyinfo.ext'] =
          'false';
      ...
      if ($scope.useRefreshTokenForClientCredentialsGrant === true) {
        $scope.clientEdit.attributes['client_credentials.use_refresh_token'] =
          'true';
      } else {
        $scope.clientEdit.attributes['client_credentials.use_refresh_token'] =
          'false';
      ...
      if ($scope.backchannelLogoutSessionRequired === true) {
        $scope.clientEdit.attributes['backchannel.logout.session.required'] =
          'true';
      } else {
        $scope.clientEdit.attributes['backchannel.logout.session.required'] =
          'false';
      ...
      if (
        $scope.clientEdit.protocol !== 'saml' &&
        !$scope.clientEdit.bearerOnly &&
        ($scope.clientEdit.standardFlowEnabled ||
          $scope.clientEdit.implicitFlowEnabled) &&
        (!$scope.clientEdit.redirectUris ||
          $scope.clientEdit.redirectUris.length === 0)
      ) {
...
module.controller(
  ...
  (
    ...
    Client,
    _ClientScope,
    ClientRealmScopeMapping,
    ClientClientScopeMapping,
    _ClientRole,
    ClientAvailableRealmScopeMapping,
    ClientAvailableClientScopeMapping,
    ClientCompositeRealmScopeMapping,
    ClientCompositeClientScopeMapping,
  ) => {
    $scope.realm = realm;
    $scope.client = angular.copy(client);
    ...
    $scope.hideRoleSelector = () => $scope.client.fullScopeAllowed;
    ...
    function updateRealmRoles() {
      $scope.realmRoles = ClientAvailableRealmScopeMapping.query({
        realm: realm.realm,
        client: client.id,
      });
      $scope.realmMappings = ClientRealmScopeMapping.query({
        realm: realm.realm,
        client: client.id,
      });
      $scope.realmComposite = ClientCompositeRealmScopeMapping.query({
        realm: realm.realm,
        client: client.id,
      });
    }
    ...
    function updateClientRoles() {
      if ($scope.selectedClient) {
        $scope.clientRoles = ClientAvailableClientScopeMapping.query({
          realm: realm.realm,
          client: client.id,
          targetClient: $scope.selectedClient.id,
        });
        $scope.clientMappings = ClientClientScopeMapping.query({
          realm: realm.realm,
          client: client.id,
          targetClient: $scope.selectedClient.id,
        });
        $scope.clientComposite = ClientCompositeClientScopeMapping.query({
          realm: realm.realm,
          client: client.id,
          targetClient: $scope.selectedClient.id,
        });
    ...
    $scope.addRealmRole = () => {
      ...
      $http
        .post(
          `${authUrl}/admin/realms/${realm.realm}/clients/${client.id}/scope-mappings/realm`,
    ...
    $scope.addClientRole = () => {
      ...
      $http
        .post(
          `${authUrl}/admin/realms/${realm.realm}/clients/${client.id}/scope-mappings/clients/${$scope.selectedClient.id}`,
...
module.controller(
  'ClientRevocationCtrl',
  (
    $scope,
    realm,
    client,
    Client,
    ClientPushRevocation,
    _$location,
    _Dialog,
    Notifications,
  ) => {
    ...
    $scope.setNotBeforeNow = () => {
      $scope.client.notBefore = Date.now() / 1000;
      Client.update(
        { realm: realm.realm, client: $scope.client.id },
        $scope.client,
        () => {
...
module.controller(
  ...
  (
    ...
    $scope.client.nodeReRegistrationTimeoutUnit = TimeUnit.autoUnit(
      client.nodeReRegistrationTimeout,
    );
    $scope.client.nodeReRegistrationTimeout = TimeUnit.toUnit(
      client.nodeReRegistrationTimeout,
      $scope.client.nodeReRegistrationTimeoutUnit,
    );

    $scope.save = () => {
      var clientCopy = angular.copy($scope.client);
      delete clientCopy['nodeReRegistrationTimeoutUnit'];
      clientCopy.nodeReRegistrationTimeout = TimeUnit.toSeconds(
        $scope.client.nodeReRegistrationTimeout,
        $scope.client.nodeReRegistrationTimeoutUnit,
      );
      Client.update(
        { realm: realm.realm, client: client.id },
    ...
    $scope.testNodesAvailable = () => {
      ClientTestNodesAvailable.get(
        { realm: realm.realm, client: client.id },
        (globalReqResult) => {
          $route.reload();
          ...
          if (successCount === 0 && failedCount === 0) {
            ...
            var msgStart =
              successCount > 0
                ? `Successfully verify availability for ${globalReqResult.successRequests} . `
                : '';
            Notifications.error(
              `${msgStart}Failed to verify availability for: ${globalReqResult.failedRequests}. Fix or unregister failed cluster nodes and try again`,
            );
          } else {
            Notifications.success(
              `Successfully sent requests to: ${globalReqResult.successRequests}`,
            );
          }
        },
      );
    ...
    $scope.removeNode = (node) => {
      Dialog.confirmDelete(node.host, 'node', () => {
        ClientClusterNode.remove(
          { realm: realm.realm, client: client.id, node: node.host },
          () => {
            Notifications.success(
              `Node ${node.host} unregistered successfully.`,
            );
            $route.reload();
          },
        );
...
module.controller(
  ...
  (
    ...
    $scope.save = () => {
      ClientClusterNode.save(
        { realm: realm.realm, client: client.id, node: $scope.node.host },
        () => {
          Notifications.success(
            `Node ${$scope.node.host} registered successfully.`,
          );
          $location.url(
            `/realms/${realm.realm}/clients/${client.id}/clustering`,
          );
        },
      );
    };

    $scope.unregisterNode = () => {
      Dialog.confirmDelete($scope.node.host, 'node', () => {
        ClientClusterNode.remove(
          { realm: realm.realm, client: client.id, node: $scope.node.host },
          () => {
            Notifications.success(
              `Node ${$scope.node.host} unregistered successfully.`,
            );
            $location.url(
              `/realms/${realm.realm}/clients/${client.id}/clustering`,
            );
...
module.controller(
  ...
  (
    ...
    var updateMappers = () => {
      var clientMappers = ClientProtocolMappersByProtocol.query(
        { realm: realm.realm, client: client.id, protocol: client.protocol },
        () => {
          var builtinMappers =
            serverInfo.builtinProtocolMappers[client.protocol];
          for (var i = 0; i < clientMappers.length; i++) {
            for (var j = 0; j < builtinMappers.length; j++) {
              if (
                builtinMappers[j].name === clientMappers[i].name &&
                builtinMappers[j].protocolMapper ===
                  clientMappers[i].protocolMapper
              ) {
    ...
    $scope.add = () => {
      ...
      $http
        .post(
          `${authUrl}/admin/realms/${realm.realm}/clients/${client.id}/protocol-mappers/add-models`,
          toAdd,
        )
        .then(() => {
...
module.controller(
  ...
  (
    ...
    $scope.removeMapper = (mapper) => {
      console.debug(mapper);
      Dialog.confirmDelete(mapper.name, 'mapper', () => {
        ClientProtocolMapper.remove(
          { realm: realm.realm, client: client.id, id: mapper.id },
          () => {
    ...
    $scope.sortMappersByPriority = (mapper) =>
      $scope.mapperTypes[mapper.protocolMapper].priority;

    var updateMappers = () => {
      $scope.mappers = ClientProtocolMappersByProtocol.query({
        realm: realm.realm,
        client: client.id,
...
module.controller(
  ...
  (
    ...
    */
    ...
    $scope.save = () => {
      ClientProtocolMapper.update(
        ...
        () => {
          ...
          $location.url(
            `/realms/${realm.realm}/clients/${client.id}/mappers/${$scope.model.mapper.id}`,
          );
    ...
    $scope.remove = () => {
      Dialog.confirmDelete($scope.model.mapper.name, 'mapper', () => {
        ClientProtocolMapper.remove(
          { realm: realm.realm, client: client.id, id: $scope.model.mapper.id },
          () => {
            Notifications.success('The mapper has been deleted.');
            $location.url(
              `/realms/${realm.realm}/clients/${client.id}/mappers`,
            );
          },
        );
      });
      ...
      () => {
        ...
        if (currentMapperType && Array.isArray(currentMapperType.properties)) {
          for (var i = 0; i < currentMapperType.properties.length; i++) {
            ...
            if (property?.name && property.defaultValue) {
    ...
    $scope.save = () => {
      ...
      ClientProtocolMapper.save(
        ...
        (_data, headers) => {
          ...
          var id = l.substring(l.lastIndexOf('/') + 1);
          $location.url(
            `/realms/${realm.realm}/clients/${client.id}/mappers/${id}`,
          );
...
module.controller(
  ...
  (
    ...
    clientScopes,
    _serverInfo,
    clientDefaultClientScopes,
    ClientDefaultClientScopes,
    clientOptionalClientScopes,
    ClientOptionalClientScopes,
    $route,
    Notifications,
    _$location,
    ...
    $scope.addDefaultClientScope = () => {
      $scope.selectedDefaultClientScopesToAdd = JSON.parse(
        `[${$scope.selectedDefaultClientScopes}]`,
      );
    ...
    $scope.deleteDefaultClientScope = () => {
      $scope.selectedDefDefaultClientScopesToRemove = JSON.parse(
        `[${$scope.selectedDefDefaultClientScopes}]`,
      );
      ...
      for (
        var i = 0;
        i < $scope.selectedDefDefaultClientScopesToRemove.length;
        i++
      ) {
    ...
    $scope.addOptionalClientScope = () => {
      $scope.selectedOptionalClientScopesToAdd = JSON.parse(
        `[${$scope.selectedOptionalClientScopes}]`,
      );
      ...
      for (
        var i = 0;
        i < $scope.selectedOptionalClientScopesToAdd.length;
        i++
      ) {
    ...
    $scope.deleteOptionalClientScope = () => {
      $scope.selectedDefOptionalClientScopesToRemove = JSON.parse(
        `[${$scope.selectedDefOptionalClientScopes}]`,
      );
      ...
      for (
        var i = 0;
        i < $scope.selectedDefOptionalClientScopesToRemove.length;
        i++
      ) {
...
module.controller(
  ...
  (
    ...
    _Realm,
    User,
    ClientEvaluateProtocolMappers,
    ClientEvaluateGrantedRoles,
    ClientEvaluateNotGrantedRoles,
    ClientEvaluateGenerateExampleAccessToken,
    ClientEvaluateGenerateExampleIDToken,
    ClientEvaluateGenerateExampleUserInfo,
    realm,
    client,
    clients,
    _clientScopes,
    ...
    $scope.addAppliedClientScope = () => {
      $scope.selectedClientScopesToAdd = JSON.parse(
        `[${$scope.selectedClientScopes}]`,
      );
    ...
    $scope.deleteAppliedClientScope = () => {
      $scope.selectedDefClientScopesToRemove = JSON.parse(
        `[${$scope.selectedDefClientScopes}]`,
      );
    ...
    $scope.sendEvaluationRequest = () => {
      ...
      if (
        ...
        var accessTokenUrl =
          ClientEvaluateGenerateExampleAccessToken.url(exampleRequestParams);
        getPrettyJsonResponse(accessTokenUrl).then((result) => {
          $scope.oidcAccessToken = result;
        });

        var idTokenUrl =
          ClientEvaluateGenerateExampleIDToken.url(exampleRequestParams);
        getPrettyJsonResponse(idTokenUrl).then((result) => {
          $scope.oidcIDToken = result;
        });

        var userInfoUrl =
          ClientEvaluateGenerateExampleUserInfo.url(exampleRequestParams);
        getPrettyJsonResponse(userInfoUrl).then((result) => {
    ...
    $scope.isResponseAvailable = () => $scope.protocolMappers != null;

    $scope.isAccessTokenAvailable = () => $scope.oidcAccessToken != null;

    $scope.isIDTokenAvailable = () => $scope.oidcIDToken != null;

    $scope.isUserInfoAvailable = () => $scope.oidcUserInfo != null;
    ...
    $scope.protocolMappersShown = () => $scope.selectedTab === 1;

    $scope.rolesShown = () => $scope.selectedTab === 2;

    $scope.exampleTabInfo = () => {
...
module.controller(
  'ClientScopeTabCtrl',
  (Dialog, $scope, Current, Notifications, $location) => {
    $scope.removeClientScope = () => {
      Dialog.confirmDelete($scope.clientScope.name, 'client scope', () => {
...
module.controller(
  ...
  (
    ...
    $scope.removeClientScope = (clientScope) => {
      Dialog.confirmDelete(clientScope.name, 'client scope', () => {
        ClientScope.remove(
          {
            realm: realm.realm,
  ...
  (
    ...
    function updateProperties() {
      ...
      if ($scope.clientScope.protocol) {
        $scope.protocol =
          $scope.protocols[
            $scope.protocols.indexOf($scope.clientScope.protocol)
          ];
      } else {
        $scope.protocol = $scope.protocols[0];
      }
      ...
      if ($scope.create) {
        ...
        ClientScope.update(
          ...
          () => {
            ...
            $location.url(
              `/realms/${realm.realm}/client-scopes/${clientScope.id}`,
            );
            Notifications.success(
...
module.controller(
  ...
  (
    ...
    $scope.removeMapper = (mapper) => {
      console.debug(mapper);
      Dialog.confirmDelete(mapper.name, 'mapper', () => {
        ClientScopeProtocolMapper.remove(
          { realm: realm.realm, clientScope: clientScope.id, id: mapper.id },
          () => {
            Notifications.success('The mapper has been deleted.');
            $route.reload();
          },
    ...
    var updateMappers = () => {
      $scope.mappers = ClientScopeProtocolMappersByProtocol.query({
        realm: realm.realm,
        clientScope: clientScope.id,
      ...
      Dialog.confirmDelete($scope.model.mapper.name, 'mapper', () => {
        ClientScopeProtocolMapper.remove(
          ...
          () => {
            Notifications.success('The mapper has been deleted.');
            $location.url(
              `/realms/${realm.realm}/client-scopes/${clientScope.id}/mappers`,
            );
          },
        );
      });
    ...
    $scope.save = () => {
      ...
      ClientScopeProtocolMapper.save(
        ...
        (_data, headers) => {
          ...
          var id = l.substring(l.lastIndexOf('/') + 1);
          $location.url(
            `/realms/${realm.realm}/client-scopes/${clientScope.id}/mappers/${id}`,
          );
          Notifications.success('Mapper has been created.');
        },
      );
...
module.controller(
  ...
  (
    ...
    var updateMappers = () => {
      var clientMappers = ClientScopeProtocolMappersByProtocol.query(
        {
          realm: realm.realm,
          clientScope: clientScope.id,
          protocol: clientScope.protocol,
        ...
        () => {
          var builtinMappers =
            serverInfo.builtinProtocolMappers[clientScope.protocol];
          for (var i = 0; i < clientMappers.length; i++) {
            for (var j = 0; j < builtinMappers.length; j++) {
              if (
                builtinMappers[j].name === clientMappers[i].name &&
    ...
    $scope.add = () => {
      ...
      $http
        .post(
          `${authUrl}/admin/realms/${realm.realm}/client-scopes/${clientScope.id}/protocol-mappers/add-models`,
          toAdd,
        )
        .then(() => {
          Notifications.success('Mappers added');
...
module.controller(
  ...
  (
    ...
    _ClientScope,
    Client,
    ClientScopeRealmScopeMapping,
    ClientScopeClientScopeMapping,
    _ClientRole,
    ClientScopeAvailableRealmScopeMapping,
    ClientScopeAvailableClientScopeMapping,
    ClientScopeCompositeRealmScopeMapping,
    ...
    function updateScopeRealmRoles() {
      $scope.realmRoles = ClientScopeAvailableRealmScopeMapping.query({
        realm: realm.realm,
        clientScope: clientScope.id,
      });
      $scope.realmMappings = ClientScopeRealmScopeMapping.query({
        realm: realm.realm,
        clientScope: clientScope.id,
      });
      $scope.realmComposite = ClientScopeCompositeRealmScopeMapping.query({
        realm: realm.realm,
        clientScope: clientScope.id,
      });
    }
    ...
    function updateScopeClientRoles() {
      if ($scope.selectedClient) {
        $scope.clientRoles = ClientScopeAvailableClientScopeMapping.query({
          realm: realm.realm,
          clientScope: clientScope.id,
          targetClient: $scope.selectedClient.id,
        });
        $scope.clientMappings = ClientScopeClientScopeMapping.query({
          realm: realm.realm,
          clientScope: clientScope.id,
          targetClient: $scope.selectedClient.id,
        });
        $scope.clientComposite = ClientScopeCompositeClientScopeMapping.query({
          realm: realm.realm,
          clientScope: clientScope.id,
          targetClient: $scope.selectedClient.id,
        });
    ...
    $scope.addRealmRole = () => {
      ...
      $http
        .post(
          `${authUrl}/admin/realms/${realm.realm}/client-scopes/${clientScope.id}/scope-mappings/realm`,
          $scope.selectedRealmRolesToAdd,
        )
        .then(() => {
          updateScopeRealmRoles();
    ...
    $scope.addClientRole = () => {
      ...
      $http
        .post(
          `${authUrl}/admin/realms/${realm.realm}/client-scopes/${clientScope.id}/scope-mappings/clients/${$scope.selectedClient.id}`,
          $scope.selectedClientRolesToAdd,
        )
        .then(() => {
          updateScopeClientRoles();
...
module.controller(
  ...
  (
    ...
    $scope.addProvider = (provider) => {
      ...
      $location.url(
        `/create/client-storage/${realm.realm}/providers/${provider.id}`,
      );
    };

    $scope.getInstanceLink = (instance) =>
      `/realms/${realm.realm}/client-storage/providers/${instance.providerId}/${instance.id}`;
    ...
    $scope.removeInstance = (instance) => {
      Dialog.confirmDelete(instance.name, 'client storage provider', () => {
        Components.remove(
          {
            realm: realm.realm,
      ...
      if ($scope.create) {
        ...
        Components.save(
          ...
          (_data, headers) => {
            ...
            $location.url(
              `/realms/${realm.realm}/client-storage/providers/${$scope.instance.providerId}/${id}`,
            );
