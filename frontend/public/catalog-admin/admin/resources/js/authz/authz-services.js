module.factory('ResourceServer', ($resource) =>
  $resource(
...
module.factory('ResourceServerResource', ($resource) =>
  $resource(
...
module.factory('ResourceServerScope', ($resource) =>
  $resource(
...
module.factory('ResourceServerPolicy', ($resource) =>
  $resource(
...
module.factory('ResourceServerPermission', ($resource) =>
  $resource(
...
module.factory('PolicyProvider', ($resource) =>
  $resource(
...
module.service('AuthzDialog', ($modal) => {
  var dialog = {};

  var openDialog = (title, message, btns, template) => {
    var controller = ($scope, $modalInstance, $sce, title, message, btns) => {
      $scope.title = title;
  ...
  dialog.confirmDeleteWithMsg = (name, type, msg, success) => {
    ...
    openDialog(title, msg, btns, '/templates/authz/kc-authz-modal.html').then(
      success,
    );
  };
...
module.factory('RoleManagementPermissions', ($resource) =>
  $resource(
...
module.factory('UsersManagementPermissions', ($resource) =>
  $resource(
...
module.factory('ClientManagementPermissions', ($resource) =>
  $resource(
...
module.factory('IdentityProviderManagementPermissions', ($resource) =>
  $resource(
...
module.factory('GroupManagementPermissions', ($resource) =>
  $resource(
...
module.factory('policyViewState', [
  () => ({
    model: {
      state: {},
    },
